...
# ===================== CHECAGEM DE VAZIO (evita erro e esconde gráficos) =====================
if df_validos.empty:
    st.markdown(
        """
        <div style="
            display:flex; align-items:center; justify-content:center;
            height:60vh; text-align:center; border:1px solid #444;
            border-radius:12px; background:#111; color:#0c6902;">
            <div style="font-size:22px; font-weight:700; line-height:1.6;">
                Selecione pelo menos um curso e um semestre<br/>
    ...
    st.stop()  # interrompe o script para não renderizar gráficos/tabelas abaixo
...
MIN_PX_MAP = {1: 12, 2: 14, 3: 16, 4: 18, 5: 20}
THRESHOLD_PCT = max(MIN_PX_MAP.keys())
MIN_LABEL_PCT = 1.0
...
total_verde = int(len(df_filtrado_geral))  # semestres selecionados (todos os cursos)
...
cores_total = ['#66c2a5', '#41ae76', '#238b45', "#0D7C3B"]
cores_curso = ["#8fafda", "#74acd4", "#278ec2", "#0D6AA0"]
...
for j, col in enumerate(ORDEM_NOTAS):
    h = df_curso_vis[col].astype(float).values
    ...
    for i in range(len(indices)):
        if h[i] > 0 and p_true[i] > 0:
            y_i = min(y[i], 99.3)
            ax2.text(x_curso[i], y_i, label_pct(p_true[i]),
...
bottom = np.zeros(len(indices), dtype=float)
for j, col in enumerate(ORDEM_NOTAS):
    h = df_total_vis[col].astype(float).values
    ...
    for i in range(len(indices)):
        if h[i] > 0 and p_true[i] > 0:
            y_i = min(y[i], 99.3)
            ax2.text(x_total[i], y_i, label_pct(p_true[i]),
                     ha="center", va="center", fontsize=8, color="black", clip_on=False)
    bottom += h
